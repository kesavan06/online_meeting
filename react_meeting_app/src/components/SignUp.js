...
export default async function SignUp(name, password, unique_name) {
    ...
    try {
        ...
        let setUser = await fetch("http://localhost:3002/signUp", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
            body: JSON.stringify({ user_name: name, password: encryptPass, unique_name: encryptUnique, user_key: secret.password }),
...
async function getRandomKey() {
    let key = await fetch("https://api.genratr.com/?length=16&uppercase&lowercase&special&numbers").then((res) => res.json());
...
async function getDetailOfAPerson(unique) {
    let getUser = await fetch("http://localhost:3002/user", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
